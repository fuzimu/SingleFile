...
class SingleFileToolbar {
	...
	async handleSaveClick() {
		try {
			...
			// 获取输入框中的文件名
			const customFilename = this.input.value.trim();
			...
			// 显示成功状态并更新输入框显示文件路径
			this.setSuccessState(response.downloadPath);
	...
	setSuccessState(downloadPath) {
		...
		// 如果有下载路径，更新输入框显示文件路径
		if (downloadPath && this.input) {
			const formattedPath = this.formatFilePath(downloadPath);
			this.input.value = formattedPath;
			this.input.style.borderColor = '#28a745';
			this.input.style.color = '#28a745';
		}
		...
		setTimeout(() => {
			...
			// 恢复输入框样式
			if (this.input) {
				this.input.style.borderColor = '#e9ecef';
				this.input.style.color = '#495057';
			}
		}, 3000);
	}
